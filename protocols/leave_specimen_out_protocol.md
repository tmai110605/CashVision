...
The empirical benchmark was executed via `scratch/run_specimen_disjoint_benchmark.py`, logged to `logs_sim/specimen_disjoint_benchmark_results.csv`, and populated in Section 3 (`Table~\ref{tab:specimen_disjoint_results}`) of the manuscript. Furthermore, empirical Wilcoxon signed-rank tests and $B=1{,}000$ cluster bootstrap confidence intervals across the 12 specimen clusters were executed via `scratch/compute_specimen_disjoint_statistics.py` and logged to `logs_sim/specimen_disjoint_statistical_results.json`, providing complete empirical statistical verification.
